...
module Nat_int : NAT = struct
  ...
  let subtract x y = if x - y > 0 then x - y else 0
...
module Nat_peano : NAT = struct
  ...
  type t = Z | S of t (*Zero and Sucessor(n)*)
  ...
  let rec add x y =
    match x with
    | Z -> y 
    | S z -> if y = Z then S z else S (add (S z) y)

  let rec multiply x y =
    match x with
    | Z -> Z
    | S z -> if y = Z then Z else add y (multiply z y)
...
module Cartesian : COMPLEX = struct
  ...
  let eq x y = if x.re = y.re then x.im = y.im else false
  let zero = {re = 0.; im = 0.}
  let one = {re = 1.; im = 0.}
  let i = {re = 0.; im = 1.}
