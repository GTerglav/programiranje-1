...
let rec intbool_reverse ibl = 
  let rec intbool_reverse' ibl acc =
    match ibl with
    | Nil -> acc
    | Int (i, xs) -> intbool_reverse' xs (Int(i, acc))
    | Bool (b, xs) -> intbool_reverse' xs (Bool(b, acc))
  in
  intbool_reverse' ibl Nil
...
let rec intbool_separate ibl = 
  let rec ib_sep ibl acc1 acc2 =
    match ibl with
    | Nil -> (reverse acc1, reverse acc2)
    | Int (i, xs) -> ib_sep xs (i :: acc1) acc2 
    | Bool (b, xs) -> ib_sep xs acc1 (b :: acc2)
  in 
  ib_sep ibl [] []
(*-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=*]
...
let update counter type_of_magic =
  match type_of_magic with
...
let rec count_magic list =
  let rec magic_counter counter = function
    ...
    | w :: ws ->
      (match w.status with
      | Newbie -> magic_counter counter ws
      | Student (magic, _) | Employed (magic, _) ->
        let counter' = update counter magic in
        magic_counter counter' ws)
  in 
  magic_counter {fire = 0; frost = 0; arcane = 0} list
